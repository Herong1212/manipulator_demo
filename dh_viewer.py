...
class URDFViewer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("DH Viewer — URDF & MDH Parameters")
    ...
    def open_set_joints_dialog(self):
        ...
        if not self.revolute_joints:
            QMessageBox.warning(
                self, "Warning", "No revolute joints available. Load a URDF first."
            )
            return
        ...
        try:
            ...
            for val in (
                ...
                else [0.0] * len(self.revolute_joints)
            ):
        ...
        def parse_and_apply():
            ...
            if len(vals) != n:
                QMessageBox.warning(
                    dialog, "Warning", f"Expected {n} values, got {len(vals)}."
                )
                return
